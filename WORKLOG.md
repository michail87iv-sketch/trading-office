...
- `Conf` в правиле bias_1D **тоже** сбрасывает в neutral (не только FakeBOS) — Conf означает «движение завершено, новый BOS ещё не сформирован».
...
- FVG-shape: используется `index` (одиночный), не `indexLeft/indexRight` — спек указывал теоретическую форму, реальная из Phase 1 была одиночным `index`.
